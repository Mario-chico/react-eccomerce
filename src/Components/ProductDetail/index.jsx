...
const ProductDetail = () => {
  ...
  return (
    <aside className={` ${context.isProductDetailOpen ? 'flex' : 'hidden'} flex-col product-detail fixed right-0 bg-white rounded-lg border justify-between border-black w-80`}>
      <div className='flex justify-between items-center p-1'>
        <h2 className='font-medium text-lg'>Detalles del producto</h2>
        <svg onClick={() => context.toggleProductDetail()} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 cursor-pointer">
          <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
        </svg>
      </div>
      <figure className='w-1/3 h-1/3 m-auto lg:w-2/4 lg:h-2/4 '>
